...
class Bootstrap extends Zend_Application_Bootstrap_Bootstrap {
	...
	protected function _initConstantes() {
		...
		Application_Service_Constantes::images();
		Application_Service_Constantes::metier();
	}
...
// 	}
	...
	protected function _initEnv() {
		...
		//configuration de l'adresse mail de l'application
		define('APP_MAIL_ADDRESS', $aApp['mail_address']);
	}

	protected function _initLdap() {
		Zend_Registry::set('ldap', $this->getOption('ldap'));
		Zend_Registry::set('ldap_query', 'LDAP_QUERY');
	}

	protected function _initState() {
		$aStates = $this->getOption('state');

		//open = true
		//closed = false
		define('STATE_RESULT', $aStates['resultats']);
		define('STATE_INSCRIPTIONS', $aStates['inscriptions']);
		define('AUTH_SHOWN', $aStates['auth_shown']);
	}
...
//		Zend_Debug::dump($nls);exit;
	...
	protected function _initAcl() {
		...
		Zend_View_Helper_NavigatioN_HelperAbstract::setDefaultRole(Projet_Acl_Acl::$default_role);
	...
	protected function _initHelpers() {
		...
		Zend_Controller_Action_HelperBroker::addPath('Projet/Controller/Action/Helper', 'Projet_Controller_Action_Helper');
	...
	protected function _initViewRenderer() {
		...
		$oViewRenderer->view->headLink(array('rel' => 'icon',
											 'href' => GABARIT_PATH . '/images/favicon.png',
											 'type' => 'image/png'),
									   'PREPEND');
		...
		$oViewRenderer->view->headLink()->appendStylesheet(STYLES_PATH.'/Layout.css')
										...
										->appendStylesheet(STYLES_PATH.'/Boutons.css')
										->appendStylesheet(STYLES_PATH.'/Text.css')
										->appendStylesheet(STYLES_PATH.'/Tables.css');
		...
		$oViewRenderer->view->navigation($oMenu->getMenu());
// 		$oViewRenderer->view->navigation()->setDefaultRole('1');
	...
	protected function _initFrankiz() {
		$aFkz = $this->getOption('fkz');

		$oFkz = new Projet_Controller_Action_Helper_Frankiz($aFkz['key'], $aFkz['site'], $aFkz['urlRemote'], $aFkz['request']);
		Zend_Controller_Action_HelperBroker::addHelper($oFkz);
// 		die(var_dump($aFkz));
	}
