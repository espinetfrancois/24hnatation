...
class Projet_Form_Decorator_RemAjBoutons extends Projet_Form_Decorator_Abstract {
	...
	protected function render_element() {
		...
		$oSpan = new Projet_Xml('div', array('class' => 'RemAjBouton'));
		$oSpan->setAttr('id', $sName.'-Boutons');
		$oSpan->setAttr('style', 'display:inline-block;');
		//on ajoute les boutons ajouter et supprimer
		$oSpan->append($this->createBouton(self::AJ, $sName));
		$oSpan->append($this->createBouton(self::RM, $sName));
	...
	protected function createBouton($sType, $sName) {
		...
		$oBouton = new Projet_Xml('span', array('class' => CSS_BOUTON.' '.$sType.self::BUT, 'id' => $sType.$sName, 'name' => $sName), $sLabel);
		return $oBouton->render();
