...
#	const DATE_FIN = "01/01/2038";
	 ...
	 */
	public function save(Projet_Entite $oEntite, $bUpdateEntity = true) {

		// si l'id n'existe pas, alors l'entité n'existe pas en base
		if (!$oEntite->getId() || $oEntite->getId() == null) {
			// donc c'est un ajout
...
#			$aTabl['PER'] = $this->getDbTable()->PDO_datetimeFormat(self::DATE_FIN);//on met la date de péremption à loin dans le temps
	...
	public function delete(Projet_Entite $oEntite) {
// 		$this->changerActivite($oEntite, array('per' => $this->getDbTable()->PDO_datetimeFormat()));
		$this->deleteRaw($oEntite->getId());
	}

	public function deleteRaw($id) {
		$this->getDbTable()->delete($this->getDbTable()->getAdapter()->quoteInto('ID = ?', $id));
	}
	...
	protected function listerIdPair($prop) {
		return $this->listerPair('ID', $prop);
	}

	protected function listerPair($cle, $prop) {
